...
//			fprintf(cache_fd, "Added to Parent has other subdirs\n");
	...
	subdir = filecache->dir_cache;
	do
	{
		/* Prevent the next entry in the linked list of unknown dirs from being nulled */ 
		nextdir = (subdir != NULL) ? subdir->next : NULL;

		if (subdir != NULL) {
			dsmcc_cache_attach_dir(filecache, dir, subdir);

			subdir = ((subdir->next == NULL) && (nextdir != NULL)) 
				? nextdir	/* The link was clobbered, restore it */
				: subdir->next;
		}
	} while (nextdir != NULL);
...
dsmcc_cache_file(struct cache *filecache, struct biop_message *bm, struct cache_module_data *cachep) {
	...
	if (file == NULL) {
		if (filecache->debug_fd != NULL) {
			fprintf(filecache->debug_fd, "[libcache] Unknown file %ld/%d/%d/%d-%d-%d-%d, caching data\n", cachep->carousel_id, cachep->module_id, bm->hdr.objkey_len, bm->hdr.objkey[0], bm->hdr.objkey[1], bm->hdr.objkey[2], bm->hdr.objkey[3]);
		}
