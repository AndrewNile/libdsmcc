...
dsmcc_cache_dir_find(struct cache *filecache, unsigned long car_id, unsigned short module_id, unsigned int key_len, char *key) {
	...
	if (module_id == 0 && key_len == 0) {
		...
		if (filecache->gateway == NULL) {
			...
			for (fdir = filecache->dir_cache; fdir != NULL; fdir = nd) {
				nd = fdir->next;
...
dsmcc_cache_attach_dir(struct cache *filecache, struct cache_dir *root, struct cache_dir *dir)
{
	...
	/* If either of these is null it's a bug, but prevent a crash at least */
	if ((dir == NULL) || (root == NULL)) {
		return;
	}

	if ((dir->carousel_id == root->carousel_id) && (dir->p_module_id == root->module_id) &&
...
dsmcc_cache_scan_file(struct cache_dir *dir, unsigned long car_id, unsigned int mod_id, unsigned int key_len, char *key) {
	...
	for (file = dir->files; file != NULL; file = file->next) {
		if ((file->carousel_id == car_id) && (file->module_id == mod_id) &&
...
dsmcc_cache_file_find(struct cache *filecache, unsigned long car_id, unsigned short module_id, unsigned int key_len, char *key) {
	...
	for (file = filecache->file_cache; file != NULL; file = file->next) {
		if ((file->carousel_id == car_id) && (file->module_id == module_id) &&
				dsmcc_cache_key_cmp(file->key, key, file->key_len, key_len)) {
					return file;
		}
	}
...
dsmcc_cache_dir_info(struct cache *filecache, unsigned short module_id, unsigned int objkey_len, char *objkey, struct biop_binding *bind) {
	struct cache_dir *dir, *last, *subdir, *nd;
	struct cache_file *file, *nf;
	...
	if (dir != NULL) {  /* Already got (check version TODO) */
		return;
	}
	...
	memset(dir->key, 0, (dir->key_len < 4) ? 4 : dir->key_len);
	...
	for (subdir = filecache->dir_cache; subdir != NULL; subdir = nd) {
		nd = subdir->next;
	...
	if ((dir->parent != NULL) && (dir->parent->dirpath != NULL)) {
		dsmcc_cache_write_dir(filecache, dir);
	}
...
dsmcc_cache_file(struct cache *filecache, struct biop_message *bm, struct cache_module_data *cachep) {
	...
	if (file == NULL) {
		if (filecache->debug_fd != NULL) {
			fprintf(filecache->debug_fd, "[libcache] Unknown file %ld/%d/%d/%d-%d-%d-%d, caching data\n", 
				cachep->carousel_id, cachep->module_id, bm->hdr.objkey_len, 
				bm->hdr.objkey[0], bm->hdr.objkey[1], bm->hdr.objkey[2], bm->hdr.objkey[3]);
		}
		...
		memset(file->key, 0, (file->key_len < 4) ? 4 : file->key_len);
		...
		if (filecache->debug_fd != NULL) {
			fprintf(filecache->debug_fd, "[libcache] Data for file %s (%ld/%d/%d/%d-%d-%d-%d)\n", 
				file->filename, file->carousel_id, file->module_id, file->key_len,
...
dsmcc_cache_unknown_file_info(struct cache *filecache, struct cache_file *newfile) {
	...
	if (filecache->file_cache == NULL) {
		...
		filecache->file_cache->next = NULL;
		filecache->file_cache->prev = NULL;
	}
	else {
...
dsmcc_cache_file_info(struct cache *filecache, unsigned short mod_id, unsigned int key_len, char *key, struct biop_binding *bind)
{
	...
	if (newfile == NULL) {
		...
		memset(newfile->key, 0, (newfile->key_len < 4) ? 4 : newfile->key_len);
	...
	if (dir == NULL) {
		...
		memset(newfile->p_key, 0, (newfile->p_key_len < 4) ? 4 : newfile->p_key_len);
		memcpy(newfile->p_key, key, key_len);
		newfile->parent = NULL;
		if (filecache->debug_fd != NULL) {
			fprintf(filecache->debug_fd, "[libcache] Caching info for file %s with unknown parent dir (file info - %ld/%d/%d/%d-%d-%d-%d)\n", 
			newfile->filename, newfile->carousel_id, newfile->module_id, newfile->key_len, 
			newfile->key[0], newfile->key[1], newfile->key[2], newfile->key[3]);
		}
		...
		memset(newfile->p_key, 0, (dir->key_len < 4) ? 4 : dir->key_len);
